...
description = '_plate_with_hole';
fname = ['deformed_u_displacement', description];
...
for SavedStep = 1 : loops
    ...
    dmin(SavedStep) = min(u_field);
    dmax(SavedStep) = max(u_field);
...
for SavedStep = step : step : loops
    ...
    scatter3(deformed(:,1), deformed(:,2), u_field * unit_m, S, u_field * unit_m, 'filled')
    ...
    %title(['Displacement field $u$, Iteration = ' num2str(TimeInstant,'%.1d')], ...
